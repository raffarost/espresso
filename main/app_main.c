...
/* MAX6675 read path: require consecutive good samples before trusting; latch faults until then. */
#define TEMP_READ_OK_STREAK  3
...
/* Stuck reading: high heater demand but temp unchanged for several samples (first-boot flake). */
#define STUCK_DIAG_MIN_POWER_PCT  10
#define STUCK_DIAG_SAMPLES        3
#define STUCK_DIAG_TEMP_EPS       0.21f   /* < half MAX6675 LSB so "same" reading */
#define STUCK_DIAG_CLEAR_MAX_C    110.f   /* clear latch when temp moves inside (0, this) */
/* Stuck diag runs in Task500ms -> heatingControl (same cadence as SPI temp sample). */
...
#define OVERSHOOT_TRIM_DEMAND_PCT_THRESHOLD 50.0f  /* apply trim when control (%) is above this */
#define OVERSHOOT_TRIM_FRAC_PER_DEG        0.02f  /* per °C of cumulative stored overshoot */
#define OVERSHOOT_TRIM_FRAC_MAX            0.15f  /* max trim fraction (e.g. 0.15 => up to 15% reduction) */
#define OVERSHOOT_STORED_SUM_MAX_C         (OVERSHOOT_TRIM_FRAC_MAX / OVERSHOOT_TRIM_FRAC_PER_DEG)
#define OVERSHOOT_COLD_START_MAX_C         60.0f  /* boiler at/below this temp (C): cold start, arm learn once */
#endif
...
/* Status line: latch worst-severity issue for RainMaker (no UART). Cleared after long clean Okay. */
#define STATUS_PRI_ZERO       25
#define STATUS_PRI_UNTRUSTED  50
#define STATUS_PRI_TOO_HIGH   75
#define STATUS_PRI_STUCK      100
#define STATUS_WORST_CLEAR_OK_REPORTS  12  /* 5s Task5000ms -> ~60s of Okay before dropping parenthetical */
...
static char s_overshoot_disp_str[12];
static float overshoot_peak_stored;   /* cumulative committed overshoot (°C), NVS; clamped */
static float overshoot_excursion_pk; /* running max (temp - setpoint) this excursion while latched */
/* Two flags: overshoot_detected is sticky for the whole learn arm (blocks soft timeout once we cross
 * temp_ok_max_threshold = setpoint+TEMP_DELTA). overshoot_excursion_latched clears at temp<=setpoint
 * so we can commit or idle and still detect a later excursion; cannot merge without breaking one path. */
static bool overshoot_excursion_latched;
static float overshoot_trim_red_cached; /* fmin(per_deg * stored, max); refreshed when stored changes */
static bool overshoot_trim_red_dirty = true;
static bool overshoot_learn_allowed = false; /* armed only on cold power-up; off after commit until next cold start */
static bool overshoot_boot_temp_sampled; /* after first trusted temp sample, arm cold overshoot learn once */
static unsigned long long overshoot_soft_timer_start_us; /* 0 until first temp >= setpoint while learn armed */
...
static void overshoot_discard_cycle_on_stuck_temp(void);
...
static void nvs_read_overshoot_peak(nvs_handle_t h);
static void nvs_persist_overshoot_peak(void);
static void overshoot_trim_red_refresh(void);
...
#if OVERSHOOT_DETECT_ENABLE
    if (strcmp(esp_rmaker_param_get_name(param), "Reset overshoot") == 0)
    {
        if (val.val.b) {
            overshoot_peak_stored = 0.f;
            overshoot_trim_red_dirty = true;
            ...
            nvs_persist_overshoot_peak();
            ESP_LOGI(TAG, "Overshoot learn reset (pkOsm cleared)");
...
#if OVERSHOOT_DETECT_ENABLE
        nvs_read_overshoot_peak(nvs_handle);
#endif
...
void nvsWrite(void)
{
    ...
    else
    {
        ...
        /* pkOsm: only written from nvs_persist_overshoot_peak() on learn commit — do not mirror here
         * (brew nvsWrite could overwrite NVS with a stale RAM value). */
...
{
    int32_t milli = 0;
    if (nvs_get_i32(h, "pkOsm", &milli) == ESP_OK && milli >= 0 && milli <= 500000) {
        overshoot_peak_stored = fminf((float)milli * 0.001f, OVERSHOOT_STORED_SUM_MAX_C);
    } else {
        overshoot_peak_stored = 0.f;
    }
    overshoot_trim_red_dirty = true;
...
static void nvs_persist_overshoot_peak(void)
{
    ...
    esp_err_t e = nvs_set_i32(h, "pkOsm", (int32_t)lroundf(overshoot_peak_stored * 1000.0f));
    if (e == ESP_OK) {
        e = nvs_commit(h);
    }
    if (e != ESP_OK) {
        ESP_LOGW(TAG, "pkOsm NVS save failed: %s", esp_err_to_name(e));
    }
    nvs_close(h);
}
...
static void overshoot_trim_red_refresh(void)
{
    if (!overshoot_trim_red_dirty) {
        return;
    }
    overshoot_trim_red_cached = fminf(OVERSHOOT_TRIM_FRAC_PER_DEG * overshoot_peak_stored,
                                        OVERSHOOT_TRIM_FRAC_MAX);
    overshoot_trim_red_dirty = false;
}

static void overshoot_apply_trim_to_control(int *p_control)
{
    overshoot_trim_red_refresh();
    float cf = (float)*p_control;
    if (cf > (float)OVERSHOOT_TRIM_DEMAND_PCT_THRESHOLD) {
        cf *= (1.f - overshoot_trim_red_cached);
...
static void overshoot_learn_try_arm_cold(void)
{
    if (!powerOn || tempCelsius <= 0.5f || tempCelsius > OVERSHOOT_COLD_START_MAX_C) {
        return;
    }
    overshoot_learn_allowed = true;
    overshoot_excursion_latched = false;
...
/* Drop in-flight warmup overshoot learn when stuck-temp diag latches (do not touch NVS trim). */
static void overshoot_discard_cycle_on_stuck_temp(void)
{
    overshoot_excursion_latched = false;
    overshoot_excursion_pk = 0.f;
...
static void overshoot_peak_detector_update(void)
{
    ...
    if (!overshoot_detected) {
        if (overshoot_soft_timer_start_us == 0ULL && tempCelsius >= (float)tempSetpoint) {
            ...
            overshoot_excursion_pk = 0.f;
            ESP_LOGI(TAG,
                     "Overshoot learn disarmed: no temp above setpoint+%d C within %.0fs after setpoint",
    ...
    if (tempCelsius <= (float)tempSetpoint) {
        if (overshoot_excursion_latched) {
            if (overshoot_excursion_pk > 0.01f) {
                const float add = overshoot_excursion_pk;
                overshoot_peak_stored = fminf(overshoot_peak_stored + add, OVERSHOOT_STORED_SUM_MAX_C);
                overshoot_trim_red_dirty = true;
                ESP_LOGI(TAG, "Warmup overshoot +%.2f degC, cumulative stored %.2f degC (trim)",
                         (double)add, (double)overshoot_peak_stored);
                nvs_persist_overshoot_peak();
                overshoot_learn_allowed = false;
                overshoot_soft_timer_start_us = 0;
                overshoot_detected = false;
...
{
    /*
     * Learn armed, not yet latched above setpoint+TEMP_DELTA: show stored trim (NVS) so reboot
     * / re-arm does not look like zero learn. Once latched, show live excursion peak; after
     * commit (learn disarmed), show stored trim again.
     */
    float disp;
    if (overshoot_learn_allowed) {
        disp = overshoot_excursion_latched ? overshoot_excursion_pk : overshoot_peak_stored;
    } else {
        disp = overshoot_peak_stored;
    }
    if (disp <= 0.f) {
        snprintf(s_overshoot_disp_str, sizeof(s_overshoot_disp_str), "0.0");
    } else {
        snprintf(s_overshoot_disp_str, sizeof(s_overshoot_disp_str), "%.1f", (double)disp);
    }
}
...
static void temp_stuck_diag_update(int heating_demand_pct)
{
    ...
    if (fabsf(t - temp_stuck_prev_c) < STUCK_DIAG_TEMP_EPS) {
        ...
        if (temp_stuck_same_ct >= STUCK_DIAG_SAMPLES) {
            ...
            ESP_LOGW(TAG, "Boiler temp stuck diag: demand >= %d%%, %d identical samples (~%.2f C)",
                     STUCK_DIAG_MIN_POWER_PCT, STUCK_DIAG_SAMPLES, (double)t);
#if OVERSHOOT_DETECT_ENABLE
            overshoot_discard_cycle_on_stuck_temp();
#endif
        }
    } else {
        temp_stuck_same_ct = 1u;
...
void app_main()
{
...
#if OVERSHOOT_DETECT_ENABLE
    snprintf(s_overshoot_disp_str, sizeof(s_overshoot_disp_str), "0.0");
    /* Live peak during warmup learn; stored cumulative trim after first commit this session */
    overshoot_disp_param = esp_rmaker_param_create("Overshoot learn (\xc2\xb0""C)", NULL, esp_rmaker_str(s_overshoot_disp_str),
                                                   PROP_FLAG_READ);
    ...
    esp_rmaker_param_t *reset_os_param = esp_rmaker_param_create("Reset overshoot", NULL, esp_rmaker_bool(false),
                                                                 PROP_FLAG_READ | PROP_FLAG_WRITE);
...
#if OVERSHOOT_DETECT_ENABLE
        if (temp_read_trusted) {
            if (!overshoot_boot_temp_sampled && tempCelsius > 0.5f) {
                overshoot_boot_temp_sampled = true;
                overshoot_learn_try_arm_cold();
            }
            overshoot_peak_detector_update();
        }
        overshoot_apply_trim_to_control(&control);
#endif
